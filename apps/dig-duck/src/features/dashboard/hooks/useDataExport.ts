...
export const useDataExport = () => {
  ...
  const exportToPDF = async ({ data, searchParams }: ExportData) => {
    ...
    try {
      ...
      yPosition += 10;
      doc.text(
        `Period: ${searchParams.startDate} ~ ${searchParams.endDate}`,
        20,
        yPosition,
      );
      yPosition += 10;
      ...
      if (searchParams.device) {
        doc.text(
          `Device: ${searchParams.device === "pc" ? "PC" : "Mobile"}`,
          20,
          yPosition,
        );
        yPosition += 10;
      ...
      if (searchParams.gender) {
        doc.text(
          `Gender: ${searchParams.gender === "m" ? "Male" : "Female"}`,
          20,
          yPosition,
        );
        yPosition += 10;
