...
describe('LicenseService', () => {
  ...
  describe('generateLicenseKey', () => {
    it('라이센스 키를 생성해야 함', () => {
      ...
      const licenseKey = licenseService.generateLicenseKey(userEmail, subscriptionType)
    ...
    it('동일한 입력에 대해 동일한 키를 생성해야 함', () => {
      ...
      const key1 = licenseService.generateLicenseKey(userEmail, subscriptionType)
      const key2 = licenseService.generateLicenseKey(userEmail, subscriptionType)
  ...
  describe('createLicense', () => {
    it('새 라이센스를 생성해야 함', async () => {
      ...
      const mockUser = { id: 1, email: 'test@example.com', name: '테스트 사용자', createdAt: new Date() }
      const mockSubscription = { id: 1, userId: 1, type: 'monthly', expiresAt: new Date(), isActive: true }
      
      testPrisma.$transaction.mockResolvedValue({ user: mockUser, subscription: mockSubscription })
  ...
  describe('activateDevice', () => {
    ...
    it('디바이스를 활성화해야 함', async () => {
      ...
      const result = await licenseService.activateDevice(licenseKey, deviceInfo)
