...
// 모의 훅 (실제로는 useLicenseGenerator를 import)
function useLicenseGenerator() {
  const [isGenerating, setIsGenerating] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const generateSingleLicense = async (config: LicenseConfig) => {
    ...
    try {
      ...
      if (!config.expiryDate) {
        throw new Error("만료일을 설정해주세요.");
      }

      // 모의 API 호출
      await new Promise((resolve) => setTimeout(resolve, 1500));

      console.log("라이센스 생성:", config);
      return { success: true };
    } catch (err: any) {
  ...
  const generateBulkLicenses = async (count: number, config: LicenseConfig) => {
    setIsGenerating(true);
    setError(null);

    try {
      if (count < 1 || count > 100) {
        throw new Error("생성 개수는 1~100개 사이여야 합니다.");
      }

      // 모의 API 호출
      await new Promise((resolve) => setTimeout(resolve, 2000));

      console.log(`${count}개 라이센스 대량 생성:`, config);
      return { success: true };
    } catch (err: any) {
      setError(err.message);
      return { success: false };
